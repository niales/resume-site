...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-dark-800 text-dark-100 flex flex-col">
      {/* Header */}
      <header className="bg-gradient-to-r from-dark-700 to-dark-900 text-white p-6 shadow-lg border-b border-dark-600">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-4xl font-bold">{resumeData.basics.name}</h1>
          <p className="text-xl mt-2">{resumeData.basics.title}</p>
          <p className="text-lg opacity-80 mt-1">{resumeData.basics.tagline}</p>
      ...
      {/* Navigation */}
      <nav className="bg-dark-700 shadow border-b border-dark-600">
        <div className="max-w-5xl mx-auto px-6 py-4">
          <ul className="flex space-x-8">
            <li>
              <button
                onClick={() => setActiveSection('experience')}
                className={`pb-2 font-medium transition-colors ${activeSection === 'experience' ? 'text-accent-300 border-b-2 border-accent-300' : 'text-dark-200 hover:text-accent-300'}`}
              >
                Experience
              </button>
            </li>
            <li>
              <button
                ...
                className={`pb-2 font-medium transition-colors ${activeSection === 'skills' ? 'text-accent-300 border-b-2 border-accent-300' : 'text-dark-200 hover:text-accent-300'}`}
              >
                Skills
              </button>
            </li>
            <li>
              <button
                ...
                className={`pb-2 font-medium transition-colors ${activeSection === 'education' ? 'text-accent-300 border-b-2 border-accent-300' : 'text-dark-200 hover:text-accent-300'}`}
              >
                Education
              </button>
            </li>
            <li>
              <button
                ...
                className={`pb-2 font-medium transition-colors ${activeSection === 'projects' ? 'text-accent-300 border-b-2 border-accent-300' : 'text-dark-200 hover:text-accent-300'}`}
              >
                Projects
              </button>
            </li>
      ...
      <main className="flex-grow max-w-5xl mx-auto px-6 py-8 w-full">
        ...
        {activeSection === 'experience' && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-dark-100 mb-6">Professional Experience</h2>
            <div className="space-y-6">
              {resumeData.experience.map((job) => (
                <div key={job.id} className="bg-dark-700 rounded-lg shadow overflow-hidden border border-dark-600">
                  <div
                    className="p-4 cursor-pointer flex justify-between items-center"
                    onClick={() => toggleJob(job.id)}
                  >
                    <div>
                      <h3 className="text-xl font-bold text-dark-100">{job.title}</h3>
                      <p className="text-dark-200">{job.company}, {job.location}</p>
                      <p className="text-sm text-dark-300">{job.period}</p>
                    </div>
                    <svg
                      className={`w-6 h-6 text-dark-300 transform transition-transform ${expandedJob === job.id ? 'rotate-180' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    ...
                    <div className="px-4 pb-4 border-t border-dark-600 pt-3">
                      <ul className="space-y-2 list-disc pl-6">
                        {job.highlights.map((highlight, index) => (
                          <li key={index} className="text-dark-200">{highlight}</li>
                        ))}
                      </ul>
                    </div>
                  )}
        ...
        {activeSection === 'skills' && (
          <div>
            <h2 className="text-2xl font-bold text-dark-100 mb-6">Skills</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {resumeData.skills.map((skillGroup, groupIndex) => (
                <div key={groupIndex} className="bg-dark-700 rounded-lg shadow p-6 border border-dark-600">
                  <h3 className="text-lg font-bold mb-4 text-dark-100">{skillGroup.category}</h3>
                  <div className="flex flex-wrap gap-2">
                    {skillGroup.items.map((skill, skillIndex) => (
                      <span
                        key={skillIndex}
                        className={`${skillColors[groupIndex % skillColors.length]} text-white px-3 py-1 rounded-full text-sm`}
                      >
                        {skill}
                      </span>
                    ))}
        ...
        {activeSection === 'education' && (
          <div>
            <h2 className="text-2xl font-bold text-dark-100 mb-6">Education & Certifications</h2>

            <div className="space-y-6">
              {resumeData.education.map((edu, index) => (
                <div key={index} className="bg-dark-700 rounded-lg shadow p-6 border border-dark-600">
                  ...
                  {edu.gpa && <p className="text-sm font-medium text-accent-300 mb-3">{edu.gpa}</p>}

                  {edu.courses && (
                    <div className="mt-4">
                      <h4 className="text-md font-medium text-dark-200 mb-2">Graduate Courses</h4>
                      <div className="flex flex-wrap gap-2">
                        {edu.courses.map((course, courseIndex) => (
                          <span
                            key={courseIndex}
                            className="bg-dark-600 text-accent-300 px-3 py-1 rounded-full text-sm"
                          >
                            {course}
                          </span>
                        ))}
              ...
              {resumeData.certifications.length > 0 && (
                <div className="bg-dark-700 rounded-lg shadow p-6 border border-dark-600">
                  <h3 className="text-xl font-bold text-dark-100 mb-4">Certifications</h3>
                  {resumeData.certifications.map((cert, index) => (
                    <div key={index} className="mb-4 last:mb-0">
                      <p className="font-medium text-dark-100">{cert.title}</p>
        ...
        {activeSection === 'projects' && (
          <div>
            <h2 className="text-2xl font-bold text-dark-100 mb-6">Projects</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {resumeData.projects.map((project, index) => (
                <div key={index} className="bg-dark-700 rounded-lg shadow overflow-hidden border border-dark-600">
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-dark-100 mb-2">{project.name}</h3>
      ...
      {/* Footer */}
      <footer className="bg-dark-900 text-dark-200 py-6 border-t border-dark-700 mt-auto">
